...
C
     ...
     >                    * WW(J,I) / (ENER(JY)-ENER(JY-1))
        ...
        ELSE
          IF (IE .EQ. 1) THEN
C           SIGA(I,K,JY) = SIGA(I,K,JY) + SIGG * 2.*ETA*DETJ/EDEL(1)**2
     ...
     >                     * DETJ/EDEL(IE)
            SIGA(I,KK,JY)= SIGA(I,KK,JY) + SIGG 
     >                 * (1.0-FF)*DETJ/EDEL(IEE)
          ENDIF
          WAG=WW(J,I)-THI(1,I)
...
C
     ...
     >                  * WTH1 / (ENER(JY)-ENER(JY-1))
        ELSE
          IF (KK .GE. KUK) KUK = KK
          IF (IE .EQ. 1) THEN
C           SIGA(I,K,JY) = SIGA(I,K,JY)+2.*ETA*SIGI(II)*DETJ/EDEL(1)**2
